...
class tx_pobdir_pi1 extends tslib_pibase {
  ...
  public $extKey = 'pobdir';	// The extension key.
  
  public $collation = 'UTF-8'; 
  
  public $prefixId = 'tx_pobdir_pi1';		// Same as class name
  
  public $extHome = '/typo3conf/ext/pobdir/';
	
  public $scriptRelPath = 'pi1/class.tx_pobdir_pi1.php';	// Path to this script relative to the extension dir.
 
  
  public $search_default = 'Suchbegriff';
  ...
  function makeTop()	{
    ...
    while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($ressource))	{
      ...
      $markerArray['###WWW###'] = 
        ($row['www'] && $row['detail'] == '1') ? 
            '<a href="http://' . $row[www] . '" target="_blank">' . $this->pi_getLL('www') . '</a>' : 
            '<font color="' . $this->conf[pobdir_inactive] . '"></font>';
